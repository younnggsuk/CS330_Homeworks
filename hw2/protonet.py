...
class ProtoNet:
    ...
    def __init__(self, learning_rate, log_dir, num_way, num_support, num_query):
        """Inits ProtoNet.
    ...
    def _step(self, task_batch):
        ...
        for task in task_batch:
            ...
            # 1. compute prototypes from support datas
            prototypes = []
            for class_ in range(self.num_way):
                data_indices_class = torch.where(labels_support == class_)[0].tolist()  # [num_support/num_way, 1, 28, 28]
                feats_support_class = self._network(images_support[data_indices_class]) # [num_support/num_way, 64]
                prototype_class = feats_support_class.mean(dim=0, keepdim=True)         # [1, 64]
                prototypes.append(prototype_class)
            prototypes = torch.stack(prototypes, dim=1) # [1, num_way, 64]

            # 2-1. compute probs for support data
            logits_support = []
            for class_ in range(self.num_way):
                data_indices_class = torch.where(labels_support == class_)[0].tolist()           # [num_support/num_way, 1, 28, 28]
                feats_support_class = self._network(images_support[data_indices_class])          # [num_support/num_way, 64]
                feats_support_class = feats_support_class.unsqueeze(dim=1)                       # [num_support/num_way, 1, 64]
                neg_squared_edist_class = -(feats_support_class - prototypes).pow(2).sum(dim=-1) # [num_support/num_way, num_way]
                logits_support.append(neg_squared_edist_class)
            logits_support = torch.cat(logits_support, dim=0) # [num_support, num_way]

            # 2-2. compute probs for query data
            logits_query = []
            for class_ in range(self.num_way):
                data_indices_class = torch.where(labels_query == class_)[0].tolist()           # [num_query/num_way, 1, 28, 28]
                feats_query_class = self._network(images_query[data_indices_class])            # [num_query/num_way, 64]
                feats_query_class = feats_query_class.unsqueeze(dim=1)                         # [num_query/num_way, 1, 64]
                neg_squared_edist_class = -(feats_query_class - prototypes).pow(2).sum(dim=-1) # [num_query/num_way, num_way]
                logits_query.append(neg_squared_edist_class)
            logits_query = torch.cat(logits_query, dim=0) # [num_query, num_way]
            
            # 3. compute cross entropy loss & accuracy
...
def main(args):
    ...
    protonet = ProtoNet(args.learning_rate, log_dir, args.num_way, args.num_support, args.num_query)
